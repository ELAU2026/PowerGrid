...
| 2026-05-30 | Switched from @kilocode/app-builder-db (Turso/libSQL) to local SQLite. Dual-runtime DB layer: bun:sqlite for Bun runtime (dev/sandbox), better-sqlite3 for Node.js (next build/start). Lazy proxy init prevents build-time crashes. Auto-migration on first DB access. DB file at sqlite.db (gitignored). |
